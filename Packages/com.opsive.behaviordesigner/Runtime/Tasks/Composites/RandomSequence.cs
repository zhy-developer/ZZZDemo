...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Composites
{
    ...
    public class RandomSequence : ECSCompositeTask<RandomSequenceTaskSystem, RandomSequenceComponent, RandomSequenceFlag>, IParentNode, IConditionalAbortParent, IInterruptResponder, ISavableTask, ICloneable
    {
        ...
        public object Save(World world, Entity entity)
        {
            ...
            if (randomSequenceComponent.TaskOrder.IsCreated) {
                var taskOrder = new ushort[randomSequenceComponent.TaskOrder.Length];
                for (int i = 0; i < randomSequenceComponent.TaskOrder.Length; ++i) {
                    taskOrder[i] = randomSequenceComponent.TaskOrder[i];
                }
        ...
        public void Load(object saveData, World world, Entity entity)
        {
            ...
            if (taskSaveData[1] != null) {
                ...
                randomSequenceComponent.TaskOrder = new UnsafeList<ushort>(taskOrder.Length, Allocator.Persistent);
                randomSequenceComponent.TaskOrder.Resize(taskOrder.Length);
                for (int i = 0; i < taskOrder.Length; i++) {
                    randomSequenceComponent.TaskOrder[i] = taskOrder[i];
                }
            }
    ...
    public partial struct RandomSequenceTaskSystem : ISystem
    {
        ...
        private partial struct RandomSequenceJob : IJobEntity
        {
            ...
            [BurstCompile]
            public void Execute(ref DynamicBuffer<BranchComponent> branchComponents, ref DynamicBuffer<TaskComponent> taskComponents, ref DynamicBuffer<RandomSequenceComponent> randomSequenceComponents, Entity entity)
            {
                for (int i = 0; i < randomSequenceComponents.Length; ++i) {
                    ...
                    if (taskStatus == TaskStatus.Queued) {
                        ...
                        if (!randomSequenceComponent.TaskOrder.IsCreated) {
                            ...
                            randomSequenceComponent.TaskOrder = new UnsafeList<ushort>(childCount, Allocator.Persistent);
                            randomSequenceComponent.TaskOrder.Resize(childCount);
                            var childIndex = taskComponent.Index + 1;
                            for (int j = 0; j < childCount; ++j) {
                                randomSequenceComponent.TaskOrder[j] = (ushort)childIndex;
                        ...
                        if (randomSequenceComponent.TaskOrder.Length > 1) {
                            // Lazy Fisher-Yates: only place the first entry now.
                            var randomIndex = randomSequenceComponent.RandomNumberGenerator.NextInt(randomSequenceComponent.TaskOrder.Length);
                            var element = randomSequenceComponent.TaskOrder[randomIndex];
                            randomSequenceComponent.TaskOrder[randomIndex] = randomSequenceComponent.TaskOrder[0];
                            randomSequenceComponent.TaskOrder[0] = element;
                        }
                    ...
                    if (randomSequenceComponent.ActiveRelativeChildIndex == randomSequenceComponent.TaskOrder.Length - 1 || childTaskComponent.Status == TaskStatus.Failure) {
                        ...
                        var remainingCount = randomSequenceComponent.TaskOrder.Length - nextRelativeChildIndex;
                        if (remainingCount > 1) {
                            ...
                            var element = randomSequenceComponent.TaskOrder[randomIndex];
                            randomSequenceComponent.TaskOrder[randomIndex] = randomSequenceComponent.TaskOrder[nextRelativeChildIndex];
                            randomSequenceComponent.TaskOrder[nextRelativeChildIndex] = element;
                        }

                        var nextIndex = randomSequenceComponent.TaskOrder[nextRelativeChildIndex];
        ...
        private void OnUpdate(ref SystemState state)
        {
            foreach (var (taskComponents, randomSequenceComponents) in
                ...
                for (int i = 0; i < randomSequenceComponents.Length; ++i) {
                    ...
                    var taskOrder = randomSequenceComponent.TaskOrder;
                    if (taskOrder.Length == 0 || randomSequenceComponent.ActiveRelativeChildIndex >= taskOrder.Length) {
                        continue;
                    }

                    if (taskComponent.Status == TaskStatus.Running && taskComponents[taskOrder[randomSequenceComponent.ActiveRelativeChildIndex]].Status != TaskStatus.Running) {
